...
@RequestScoped
@Path("/api/role")
@Tag(name = "role", description = "role endpoint")
@Slf4j
public class RoleAPI {
    ...
    public Response create(@Valid final Role question) {
	if (question.id != null) {
	...
	final Role entity = Role.findById(id);
	if (entity == null) {
	    throw new WebApplicationException(
    ...
    @GET
    @Path("/items/{id}")
    public Role getSingle(@PathParam(value = "id") final Long id) {
	final Role entity = Role.findById(id);
	if (entity == null) {
	    throw new WebApplicationException(
    ...
    @Transactional
    @RolesAllowed(value = { "admin" })
    public Role update(@PathParam(value = "id") final Long id,
	    final Role survey) {
	if (survey.role == null) {
	    throw new WebApplicationException(
